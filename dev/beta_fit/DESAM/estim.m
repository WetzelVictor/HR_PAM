...
% r est le nombre de fr�quences recherch�es
% si r=0, ce nombre est d�tect� automatiquement
%
...
N8 = floor(N/8);
N4 = 2*N8;
rmax = n/2; % valeur maximale admissible du param�tre r
q = 51; % longueur du filtre de rang pour �liminer les pics dans le p�riodogramme
p = 5; % longueur du filtre de blanchiment du bruit
y = [zeros(p-1,1);y(:)]; % insertion de zeros au d�but du signal
duree = p+l-1:N8:length(y)-n+1; % instants d'analyse
a = zeros(p,length(duree)); % filtre de blanchiment
win_N = hanning(N); % utilisation d'une fen�tre de Hanning pour le calcul du p�riodogramme
e = zeros(p,1);
e(1) = 1;
Np = 2^nextpow2(2*N-1);
ax = (0:Np-1)/Np;
...
for t = duree; % pour chaque fen�tre d'analyse
   x = y(t-l+1:t+n-1); % extraction d'un segment de signal
   P = abs(fft(x.*win_N, Np)).^2/N; % calcul du p�riodogramme
   ...
   end; % fin de s�lection de l'ordre
   k = k+1; % it�ration suivante
end;
...
for t = duree; % pour chaque fen�tre d'analyse
   ...
   % CALCUL DES PRINCIPAUX VECTEURS PROPRES
   H = hankel(x(1:n),x(n:N));
   if k<=2, % � l'initialisation, calcul exact
      [U,S,V] = svd(H);
      W = U(:,1:r);
   else, % ensuite, algorithme adaptatif
      A = H*(H'*W);
