function [p,h,f] = loti(M,N,ws,alpha,taille_grid)
% function [p,h,f] = loti(M,N,ws,alpha,taille_grid)
% cosine-modulated pour N impair
% p = filtre prototype
% h = filtres d'analyse
...
for i=1:N/2,
	...
	for j=1:N/2,
		if(i == j),
			Us(i,i) = pimws-sin(arg1*ws)/(2*arg1);
		else
			imj = i-j;
			jmi=-imj;
			ipj = i+j-N-1;
			Us(i,j) = sin(imj*ws)/(2*jmi) - sin(ipj*ws)/(2*ipj);
		end
	end
end
i = (N+1)/2;
for j=1:N/2,
	...
	ipj = i+j-N-1;
	Us(i,j) = sin(imj*ws)/(2*jmi) - sin(ipj*ws)/(2*ipj);
	Us(j,i) = sin(imj*ws)/(2*jmi) - sin(ipj*ws)/(2*ipj);
end
Us((N+1)/2,(N+1)/2)=pi-ws;
...
p1 = [p zeros(1,length(p)-1)];
p2 = [zeros(1,length(p)-1), fliplr(p)];
p = 0.5*(p1+p2);
p=p';
